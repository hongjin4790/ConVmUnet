...
# resnet
try:
    from ConvNext_V1_Origin import convnext_tiny
except:
    from .ConvNext_V1_Origin import convnext_tiny
...
class VSSM(nn.Module): # depths 체크하기 base는 9, 2
    def __init__(self, depths = [2,2], dims = [384, 768],depths_decoder = [2, 2],dims_decoder= [768, 384], 
        ...
        dpr_decoder = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths_decoder))][::-1] # [0.20000000298023224, 0.1666666716337204, 0.13333332538604736, 0.09999999403953552, 0.06666667014360428, 0.03333333507180214, 0.0]
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        x = self.conv1(x)
        x = self.up(x) # 중간에 업샙플링하는 이유는 더 큰 해상도에서 특징을 추출하기위해서
        x = self.conv2(x)  
...
class ConvNextVMUNet(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.segment_head = SegmentationHead(in_channels=24, out_channels=1, kernel_size=3)
...
if __name__ == '__main__':
    ...
    # model = convnext_tiny(weights='ConvNeXt_Tiny_Weights.DEFAULT').cuda()

    # fet1, fet2, fet3 = model(inputs)
    # print(fet1.shape)
    # print(fet2.shape)
    # print(fet3.shape)
    ...
    # out_channels = [192, 96, 48, 24]
    # skip_channels = [384, 192, 96, 0]
    # blocks = [(in_ch, out_ch, sk_ch) for in_ch, out_ch, sk_ch in zip(in_channels, out_channels, skip_channels)]
    # print(blocks)
